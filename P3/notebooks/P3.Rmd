...
Uma rede neural artificial funciona como um algoritmo de Machine Learning, isto é, dada uma entrada de dados e a especificação do tipo de resposta desejada, obtem-se um ajuste de sinapses/funções matemáticas e suas respectivas importâncias dentro de um "emaranhado" de conexões até que seja possível caracterizar o fenômeno em análise garantindo que o erro cometido seja mínimo.

O neurônio, ou nó, é a unidade de processamento fundamental de uma rede neural. De forma resumida, durante o processo de aprendizagem a rede neural armazena o conhecimento obtido através dos pesos de cada uma das conexões realizadas, extrai mais informações externas, aprimora suas estimativas da importância das sinapses e com isto, minimiza o erro cometido (no caso de classificação binária, minimiza-se a entropia cruzada - equação \ref{eq:ec}).
...
De modo geral, para minimizar a entropia utiliza-se o gradiente descendente no algoritmo de backpropagation. Com a função de ativação softmax e combinada com a função de perda de entropia cruzada, a rede neural é configura um modelo de regressão logística simples. 

Neste experimento, dada a resposta binária, tumor Maligno ou Benigno, câncer ou não câncer de mama, a aplicação da rede neural será aplicada com o intuito de classificação do tipo de tumor e se assemelhará, conforme mencionado, a uma regressão logística. A seguir é apresentada a reprodução do experimento.
...
Os hiper parâmetros da rede neural foram ajustados conforme o experimento original, conforme mostra a Figura \ref{fig:experimento}.

```{r experimento, echo=FALSE, fig.align='center', fig.cap = "Hiper parâmetros da Rede Neural. Fonte: Predição do Câncer de Mama com Aplicação de Modelos de Inteligência Computacional de Silva et.al (2019).", out.width="60%"}
...
treino <- df[treino_id,] #Cancer é o nome a base de dados utilizada
...
modelrn <- neuralnet(diagnostico~., data = treino, hidden = c(2,7,4), rep = 50, stepmax = 100000, act.fct = "logistic", algorithm = "backprop", learningrate = 0.001, err.fct = "sse")
```
